...
nvec = [20,40,80,160,320,640,1280];
hvec = []; errovec = [];

tic
for ref = 1:length(mvec);
    ...
    rhs(:,1) = rhs(:,1) - usoln(Iint,1)/k^2;
    rhs(:,n) = rhs(:,n) - usoln(Iint,n+2)/k^2;
    ...
    e1 = ones(m,1);
    e2 = ones(n,1);
    T = spdiags([e1/h/h (-2*e1/h/h -2*e1/k/k)  e1/h/h],[-1 0 1],m,m);
    S = spdiags([e2 e2],[-1 1],n,n);
    A = (kron(I2,T) + kron(S,I1));
...
fprintf('Tempo de execução = %s segundos \n',toc)
