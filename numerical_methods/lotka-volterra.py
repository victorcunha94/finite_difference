...
N = 100
t, h = np.linspace(0, 15.0,N, retstep=True)
...
def taylor_q1_implicit(un1, f, h):
    ...
    for n in range(0, N-1): #N - 1 pq?
        ...
        # print(G(un1[:,n]))
        print(f'jacobiana=',jacobiana(G, un1[:,n]))

        un1[:,n+1]  = newtonsys(G, jacobiana, un1[:,n])
    return un1


un1 = taylor_q1_implicit(un1, f, h)

################## PLOTAGEM ###############################
fig, ax = plt.subplots()
plt.axhline(0, color='black',linewidth=1) # Eixo X horizontal
plt.axvline(0, color='black',linewidth=1) # Eixo Y vertical
plt.plot(t, un1[0], color='red', linewidth=1, label=r'Presa')
plt.plot(t, un1[1], color='blue',linewidth=1, label=r'Predador')
plt.legend()
plt.show()
############################################################
